...
$lang = array (
  ...
  'overview_title'  => 'TheoryMine为您提供给全新数学定理起个性化的名字机会。新奇的礼物不是么？',
  'overview_tagline' => '您可以自主命名一个新生的数学定理，这些定理由世界最先进（我们希望是）的计算机定理证明程序（类似机器人数学家）生成，您可以用您的爱人，老师，朋友，自己，甚至宠物为它们命名。',
  ...
  'overview_eg' => '比如，林式定理，小李子定理',
 
  //page_discover
  
  'discover_title' => '命名一个新生的数学定理:',
  'discover_15'=> '仅15英镑',
  ...
  'discover_part2' =>'如果在订购您的数学定理后，您还购买其他的定理相关的礼品。当您收到您的定理后您客店点击在您的账户页面里的定理名旁边的“礼品中心”按钮来继续。' ,
  'discover_part3' => '注意：如果您想使用特定的邮箱来接受您的定理，请您确保使用该地址来注册和登录我们的系统。否则，我们将吧定理发送到您的Paypal的邮箱地址。',
  'discover_choose' => '选择您的定理名字:',
  'discover_eg' => '例如： Tom定理',
  'discover_tc1' => '我已阅读并同意条款和条例',
  'discover_tc2' => 'TheoryMine条款条例', 
  'discover_sub1' => '当您提交您的订单是，您将接收到您的用户名和密码来', 
  'discover_sub2' => '我们的网站。您可以随时登录来查看您订单的状态，您命名的数学定理以及下载您数学定理的证书。',
  'discover_sub3' => '数学定理由机器人数学家来探寻，我们尽我们所能来确保这些定理是未曾被发表的，同时我们保证每一条新发现的数学定理都是我们数据库中不存在的。在极少数的情况下，我们不排除有先前的文章发布了相同的数学定理的情况，如发生该情况我们将在更换一个新的数学定理之外，再额外赠送一条新的数学定理给客户。欲知更多我们如何确保我们发现的数学定理是未被发现过的，',
  ...
  'faq_a5' => '不，这也正是我们产品的魅力所在。一旦您为一个最新发现的数学定理命名，该定理将永远使用您给的名字。<br> 但是如果您在您的订单仍未生效前想更换名字，您可以马上发邮件到info@theorymine.co.uk和我们联系。', 
  ...
  'faq_a8' => '这里首先我们请您注意，如果您在支付时没有登录我们官方网站的账号。我们的系统将默认把您的数学定理相关信息发送到和paypal相关联的电子邮箱里。此时，您可以通过我们给您的paypal邮箱中确认信中的用户名和密码来登录我们的官方网站。如果您在我们承诺的时间内未收到确认信，请参阅<a href="http://www.theorymine.co.uk/?go=faq#process">该链接</a>。或者，您收到确认信后，无法通过“查看证书”来获取您的数学定理的证书，请您及时发送电子邮件到info@theorymine.co.uk 和我们联系，我们将在第一时间为您解决问题。', 
  ...
  'faq_a13' => 'TheoryMine证明函数的性质，比如加法函数中，加数于被加数的顺序是不重要的，2+3 等于 3+2。 然而，减法函数就不一样了，2-3并不等于3-2，TheoryMine研发出一些像加法一样的数学函数，并研究一些类似参数的顺序是否重要等数学函数的性质。一些函数是其他一些函数的逆运算。比如，加法和减法，加上一个数后减去同样一个数，该数将不变。TheoryMine也发掘一些函数的互逆性质。这些都是TheoryMine所涉及的经典的数学定理。', 
  ...
  'about_title' => '关于我们：',
  'about_p1' =>'Theory<font color="#b4975a">Mine</font>专注于数学定理的<a href="http://dream.inf.ed.ac.uk/events/automatheo-2010/">自动探索与发现</a>  ：我们研发人工智能技术，并将其用于探索新的数学相关概念和证明的自动化。通过提供一些有趣的数学定理的证明应用，(例如 <a href="?go=discover">命名新的数学定理</a>)，来支持逻辑推理自动化技术的相关应用。尤其，我们正致力于可用于改善软件可靠性和安全性的验证工具，以及一些能简易新的数学相关概念的探寻的工具。',
  'about_p2' =>'我们的软件基于国际社区的数十年的自动化推理的研究，尤其是<a href="http://itp2011.cs.ru.nl/ITP2011/Home.html">交互式定理证明（interactive theorem proving）</a> 以及 <a href="http://cade23.ii.uni.wroc.pl/">自动推理（automated deduction）</a>. 该研究成果配得更加广泛的应用及肯定。',
  'about_p3' =>'TheoryMine由一群在<a href="http://www.ed.ac.uk/">爱丁堡大学</a> <a href="http://www.ed.ac.uk/schools-departments/informatics/">信息学院</a>研究人员组建:',
  ...
  'about_fla' => '毕业于爱丁堡大学 人工智能与数学（Artificial Intelligence and Mathematics） 研究方向',
  ...
  'gift_title'=>'礼品包:', 
  'gift_p1' =>'当您命名了一个定理的那瞬间，您就将那名字与那永恒的定理系在了一起. ',
  'gift_p2' =>'一个数学定理的发明需要大量的工作以及聪明的思维。这就赋予了发现者命名这些定理的权利，比如Gauss, Fermat和Pythagoras。现在这种将自己的名字与那些数学家巨人的名字铭刻在一起的梦想将成为现实。首先必须感谢能自动生成并证明数学定理的系统。我们保证我们定的定理是唯一，因为他们是从唯一的数学理论中生成的。数学归纳法在设计层面上就保证了数学理论的一致性，并且不是伪真实的。',
  ...
  'tes_title' => '见证:', 
    'tes_p1' => ' ”这的的确确捕获了我的想象力，我很高兴能作为TheoryMine的第一个顾客。这真是一种苏格兰人工智能的创新的应用，创造了这么新颖的和有趣的产品！”', 
  'tes_p2' => '“我时常相想象要是能证明出一个定理，并用自己的名字来让它永远记在数学的史书上是种怎样的感觉。我一度认为这个梦想不可能实现了，因为我成为科学领域的作家。但是已经Aron\'s theorem成真了。”', 
  'tes_p3' => '“这是一个让人吃惊的创新！”', 
  'tes_p4' => '“只要15磅就在TheoryMine买个名字，比去Pythagoras改名字还便宜20磅。”', 
  'tes_p5' => '“今年的礼物是这个巧妙的命名（或者甚至可能是天才）！”', 
  'tes_p6' => '“开始享受数学永远不会太早，这是个一辈子的礼物。”', 
  'tes_p7' => '点击查看BBC\'s Material World的采访 ',
   ...
   'canc_title' => '取消条例', 
   'canc_p' => '由于您购买的TheoryMine的数学定理是专门为您定制的，所以您可以在产品未被产生前的任意时间取消。您只需要发邮件到 info@theorymine.co.uk ，我们将取消订单并退款给您。如果您的产品已经开始受理，那么您将无法取消订单并得到退款。如果您对您的定理不满意（比如定理不是唯一的），那么您可告知我们，我们将乐意退款给您。', 
  ...
  'certex_title' => '证书样品说明',
  'certex_p1' => '如下显示的是TheoryMine生成的证书的样品. 关于表达式的解释可以通过直接点击链接来查看。',
  'certex_q1' => '如何理解证书？ 概述', 
  'certex_a1' =>'<p> BBC广播4台的科学杂志节目Material World在2010年4月15日采访了我们，我们特意为记者Quentin Cooper生成了一条数学定理。该证书和原先授予Quentin的有些细微不不同，我们已经更新了样本证书的格式到现在的版本。此外，我们也已经将数学表达式命名规范化。 </p><p> 紧接在定理名Quentin\'s Theorem后的依次是：数学对象的定理<em> T</em> ; 数学函数的定义, <em> f<sub>&#945</sub></em> ;以及数学定理的申明。</p>
...
是可应用于这些数的加法函数. 该函数的参数是两个带颜色的数值，并返回一个值为两参数值的和的数值并带有第 <em> second</em> 参数的颜色.</p>
<p> 值得注意的是该加法和常规意义上的加法是不同的, <em> f<sub>&#945</sub></em>  is not
...
<p> 然而，当三个数相加是，前两个参数的顺序就无关紧要了。因为返回值的颜色值依赖于第三个参数而不是前两个参数。例如：
<em> 
...
<p>布尔值, <em> true</em>  and <em> false</em> , 可以通过退化形式的的递归来定义, 该定义式由两个基础范例（base case）组成且没有进阶范例（step case）: <em> B = true | false</em> . </p>
...
请注意在进阶范例的的特殊的组织方式：相对于头部表达式中的 <em> x</em> , 主体表达式中的 <em> x</em> 被逐步转换成更小的值的范围的定义方式。<em> N</em> 的定义方式是<em> well founded</em> ，该属性保证了该类型的成员的值不能无限减小，任意的值在减少后最终会终止于$0$。这确保了加法函数的演算重重能够终止。即，进阶范例的使用次数是有穷的，最终基础范例将可被应用，此时演算结束。为了将区别出这种函数和构造函数( <em> 0</em> 和<em> s</em> )，他们被称为<em> 已定义函数</em> 。',
	'certex_q3' => '定义四色数', 
...
<em> T</em> 里的一个值。 这被称为<em> 数学归纳法证明</em> 。 TheoryMine采用这种技术来证明生成的定理。 </p>.',
	...
	'certex_a5' => '<p> 在证书上， Quentin\'s Theorem 被声明为"通过在<em> y</em> 使用数学归纳法证明"。 这该如何理解呢？ </p>
...
<p> 此时，等式的左右边已经完全相等，而引理也已经在先前被TheoryMine证明了，该引理名字叫<em>  The Richard Scott Russell Theorem </em> 。至于证明的具体细节就留给读者了。 </p>',
	'certex_q6' => 'Variations on a theme', 
	'certex_a6' => '<p> The <a href="#data-type">四色数</a> 描述了无限多种可能的新的数学对象的其中一种可能。通过选用不同的不同基础范例和进阶范例并传入不同的值，我们可以得到不同的对象。例如，我们给<em> C<sub>a</sub></em>  只传入一个布尔类型的参数。</p>
...
<p> 而且我们不必局限于 <em> B</em>  and <em> N</em> 作为输入参数。在 <em> T</em> 的定义的基础上, 我们还可以勇气作为输入参数来构造一些新的对象<em> T\'</em> , </p>
...
<li><strong>已定义函数:</strong> 转换一个到多个输入为一个输出。在计算机专业他们也被称为 In computer science, they are also known
as <em> 程序</em>  。而这种我们用的这种递归形式的已定义函数被称为<em> 函数程序</em> . </li>
